...
public:
    ...
    vector<float> getOdometry(deque<nav_msgs::Odometry>& odomQueue, double img_time)
    {
        ...
        // printf("\033[1;33mt_odom_imu\033[0m\n");
        tf::Matrix3x3 m(t_odom_imu.getRotation());
        double roll, pitch, yaw;
        m.getRPY(roll, pitch, yaw);
        // printf("%f %f %f %f %f %f\n", t_odom_imu.getOrigin().x(), t_odom_imu.getOrigin().y(), t_odom_imu.getOrigin().z(), roll, pitch, yaw);
        
        // Rotate orientation to VINS world
        tf::Quaternion q_world_imu = tf::createQuaternionFromRPY(0, 0, M_PI) * t_odom_imu.getRotation();
        tf::quaternionTFToMsg(q_world_imu, odomCur.pose.pose.orientation);
        ...
        // printf("%f %f %f %f %f %f %f\n", odometry_channel[1], odometry_channel[2], odometry_channel[3], odometry_channel[4], odometry_channel[5], odometry_channel[6], odometry_channel[7]);
        return odometry_channel;
    }
};
