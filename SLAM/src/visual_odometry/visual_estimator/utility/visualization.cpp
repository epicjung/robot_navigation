...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header, const int& failureId)
{
    ...
    // printf("\033[1;35mIMU in VINS\033[0m\n");
    ...
    tf::quaternionTFToMsg(t_world_lidar.getRotation(), test_odom.pose.pose.orientation);
    pub_latest_odometry_ros.publish(test_odom);

    tf::Matrix3x3 mw(t_world_lidar.getRotation());
    mw.getRPY(roll, pitch, yaw);
    // printf("\033[1;32mLiDAR in vins_world\033[0m\n");
    // printf("%f %f %f; %f %f %f\n", t_world_lidar.getOrigin().x(),t_world_lidar.getOrigin().y(), t_world_lidar.getOrigin().z(), roll, pitch, yaw);
