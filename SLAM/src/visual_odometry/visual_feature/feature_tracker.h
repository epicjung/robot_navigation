...
public:
    ...
    get_depth_and_prob(const ros::Time& stamp_cur, const cv::Mat& imageCur, 
                                          ...
                                          const sensor_msgs::PointCloudPtr feature_points)
    {
        ...
        // 0.2 initialize static probability
        sensor_msgs::ChannelFloat32 prob_of_point;
        prob_of_point.name = "probability";
        prob_of_point.values.resize(features_2d.size(), -1.0); // assume static by default

        // 0.3  check if depthCloud available
        if (depthCloud->size() == 0)
            return std::make_pair(depth_of_point, prob_of_point);

        // 0.4 look up transform at current image time
        try{
        ...
        *depth_cloud_local = *depth_cloud_local_filter2;
        ...
        for (int i = 0; i < (int)features_3d_sphere->size(); ++i)
        {
            ...
            if (pointSearchInd.size() == 3 && pointSearchSqDis[2] < dist_sq_threshold)
            {
                ...
                // average static probability
                float p1 = depth_cloud_unit_sphere->points[pointSearchInd[0]].curvature;
                float p2 = depth_cloud_unit_sphere->points[pointSearchInd[1]].curvature;
                float p3 = depth_cloud_unit_sphere->points[pointSearchInd[2]].curvature;
                float average = (p1 + p2 + p3) / 3.0;
                features_3d_sphere->points[i].curvature = average;
                // printf("Id: %d \n prob: %f, %f, %f, %f\n", (int)feature_points->channels[0].values[i], p1, p2, p3, average);
                // printf("xyz: %f %f %f %f\n", depth_cloud_local->points[pointSearchInd[0]].x, depth_cloud_local->points[pointSearchInd[0]].y, depth_cloud_local->points[pointSearchInd[0]].z, depth_cloud_local->points[pointSearchInd[0]].curvature);
                // printf("xyz: %f %f %f %f\n", depth_cloud_local->points[pointSearchInd[1]].x, depth_cloud_local->points[pointSearchInd[1]].y, depth_cloud_local->points[pointSearchInd[1]].z,  depth_cloud_local->points[pointSearchInd[1]].curvature);
                // printf("xyz: %f %f %f %f\n", depth_cloud_local->points[pointSearchInd[2]].x, depth_cloud_local->points[pointSearchInd[2]].y, depth_cloud_local->points[pointSearchInd[2]].z,  depth_cloud_local->points[pointSearchInd[2]].curvature);
            }
        }
        ...
        // update depth and probability value for return
        for (int i = 0; i < (int)features_3d_sphere->size(); ++i)
        {
            // if (features_3d_sphere->points[i].intensity > 3.0)
            depth_of_point.values[i] = features_3d_sphere->points[i].intensity;
            prob_of_point.values[i] = features_3d_sphere->points[i].curvature;
        }
        ...
        if (pub_depth_image.getNumSubscribers() != 0)
        {
            ...
            for (int i = 0; i < (int)points_2d.size(); ++i)
            {
                ...
                cv::circle(circleImage, points_2d[i], 2, cv::Scalar(r, g, b), 2);
            }
