...
class Odom_sim {
    private:
        ...
        tf2_ros::StaticTransformBroadcaster tf2_Map2CostmapCenter;
    ...
    public:
        ...
        void publish_tf(){
            ...
            ROS_INFO("Odom Simulator: Sending Map2CostmapCenter TF");
            ROS_INFO("utm: %f, %f, %f, %f\n", T_odom.getOrigin().x(), T_odom.getOrigin().y(), T_odom.getRotation().z(), T_odom.getRotation().w());
            ROS_INFO("costmap: %f, %f, %f, %f\n", T_costmap.getOrigin().x(), T_costmap.getOrigin().y(), T_costmap.getRotation().z(), T_costmap.getRotation().w());
        ...
        Odom_sim(ros::NodeHandle & nh): nh_(nh){
            ...
            this->sub_ = this->nh_.subscribe("/gps/first_odom", 1, &Odom_sim::transformer, this);

            // The Global GPS position is the Global costmap's center GPS position
            double utm_nort, utm_east;
            gps_common::UTM(36.3683644417,127.362330045,&utm_nort,&utm_east); // open-field
            // gps_common::UTM(36.368994196775716, 127.3657408254035,&utm_nort,&utm_east); // EE building
        ...
        void transformer(const nav_msgs::Odometry & msg){
            if (this->once_ == true){
                ...
                utm_odom.orientation.x = msg.pose.pose.orientation.x;
                utm_odom.orientation.y = msg.pose.pose.orientation.y;
                utm_odom.orientation.z = msg.pose.pose.orientation.z;
                utm_odom.orientation.w = msg.pose.pose.orientation.w;
